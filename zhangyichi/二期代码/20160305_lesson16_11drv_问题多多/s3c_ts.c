...
//  8th  第5次优化   把打印语句printk改成上报事件就可以了，就成为一个完整
// 的触摸屏驱动程序。从优化措施4中进行更改。
...
ADC成功后，进入中断adc_irq处理函数，在该函数里确定adcdat0 = s3c_ts_regs->adcdat0
	和adcdat1 = s3c_ts_regs->adcdat1的值
	该函数内，如果松开，则再次进入等待触摸笔进入模式enter_wait_pen_down_mode()
	否则，仍继续按下的话，则保存如下值x[cnt] = s3c_ts_regs->adcdat0 & 0x3ff和
...
static struct timer_list ts_timer;   // 2个参数，时间到和处理函数
