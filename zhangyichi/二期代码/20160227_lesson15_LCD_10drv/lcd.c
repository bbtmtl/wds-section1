...
static u32 pseudo_palette[16];     //¼ÙµÄµ÷É«°å,¼û2.4
			     ...
			     unsigned int transp, struct fb_info *info)
{
	...
	if(regno >16)
		return 1;  
	/*ÓÃºì¡¢ÂÌ¡¢À¶¹¹Ôì³övalÖµ
	    chan &= 0xffff;    16Î»¶¼È¡1£¬½ö½ö±£ÁôµÍ16Î»
		chan >>= 16 - bf->length;  ¼ÙÉè³¤¶ÈÊÇ5µÄ»°£¬16-5=11Ïàµ±ÓÚÓÒÒÆ11Î»£¬Ö»±£Áô¸ß5Î»
		return chan << bf->offset; ÔÙ×óÒÆ
	*/
	val  =  chan_to_field(red,     &info->var.red);
	val |=  chan_to_field(green, &info->var.green);
	val |= chan_to_field(blue,    &info->var.blue);

	// ((u32 *)info->pseudo_palette))[regno] = val; Ç¿ÖÆ×ª»»Îªu32
	pseudo_palette[regno] = val; //·´ÕıÊÇ±¾µØµÄµ÷É«°å£¬¿ÉÒÔ¼ò»¯³ÉÕâÑùµÄĞÎÊ½
...
static int lcd_init(void)
{
	...
	s3c_lcd->var.bits_per_pixel   =       16;    //16Î» 
	...
	//2.4 ÆäËûµÄÉèÖÃ
	s3c_lcd->pseudo_palette  = pseudo_palette   ; //¼ÙµÄµ÷É«°åå 
...
static int lcd_exit(void)
{
	...
	lcd_regs->lcdcon1 &=~(1<<0); /* ¹Ø±ÕLCD±¾Éí*/
